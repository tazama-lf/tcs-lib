...
export class PayloadParsingService {
  ...
  constructor(private readonly _jsonSchemaConverter: JSONSchemaConverterService) {}
  async parsePayloadToSchema(
    ...
    try {
      ...
      const jsonSchema = this._jsonSchemaConverter.convertToJSONSchema(
        sourceFields,
        'Generated Schema',
      );
  ...
  private addStructureError(error: any, contentType: ContentType, errors: string[]): void {
  ...
  private validateJsonStructure(parsed: any, errors: string[], warnings: string[]): void {
    if (parsed === null) {
  ...
  private validateXmlStructure(xmlString: string, errors: string[], warnings: string[]): void {
  ...
  private async parsePayloadToObject(payload: string, contentType: ContentType): Promise<any> {
    if (contentType === ContentType.JSON) {
  ...
  private createSchemaField(
    ...
    if (Array.isArray(value)) {
      const children =
        value.length > 0 ? this.generateHierarchicalSchema(value[0], path, level + 1) : [];
      return {
        name,
        path,
  ...
  private validateGeneratedSchema(fields: SchemaField[]): SchemaValidationResultDto {
    const errors: string[] = [];
    const warnings: string[] = [];
  ...
  private calculateMaxNestedLevels(fields: SchemaField[], currentLevel: number = 0): number {
